...
int SaveGame(const MAP_INFO *pmap_info, const BLOCK board[][32])
{
    ...
    else
    {
        ...
        fprintf(fp, "%d\n", EncryptNum(pmap_info->map_row) + EncryptNum(pmap_info->map_col) + EncryptNum(pmap_info->map_mines) + EncryptNum(pmap_info->map_flag) + EncryptNum(pmap_info->map_open) + EncryptNum(pmap_info->run_time));
