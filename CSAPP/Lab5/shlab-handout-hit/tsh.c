...
int builtin_cmd(char **argv)
{
	if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")) /* bg or fg builtin command */
	{
		do_bgfg(argv);
		return 1;
	}
...
void waitfg(pid_t pid)
{
	struct job_t* jobp = getjobpid(jobs, pid); /* get corresponding job pointer by pid */
...
void sigchld_handler(int sig)
{
	...
	sigfillset(&mask);
	sigprocmask(SIG_BLOCK, &mask, &oldmask); /* block all signals */
	...
	{
		struct job_t* jobp = getjobpid(jobs, child_pid); /* get corresponding job pointer by pid */
		if (!jobp)
			app_error("getjobpid (sigchld_handler) error");
		
		if (WIFEXITED(status))	/* child process exited normally */
		{
			if (!deletejob(jobs, child_pid))	/* delete the job */
				app_error("deletejob (sigchld_handler) error");
		}
		else if (WIFSTOPPED(status))	/* child process stopped */
		{
			int sig = WSTOPSIG(status);	/* get the signal id which causes child process stopped */
			printf("Job [%d] (%d) stopped by signal %d\n", jobp->jid, jobp->pid, sig);
			jobp->state = ST;	/* stop the job */
		}
		else if (WIFSIGNALED(status))	/* child process terminated */
		{
			int sig = WTERMSIG(status);	/* get the signal id which causes child process terminated */
	...
	sigprocmask(SIG_SETMASK, &oldmask, NULL);	/* restore original signal mask */
...
void sigint_handler(int sig)
{
	...
	if (fg != 0)
	{
		if (kill(-fg, SIGINT) < 0)	/* send SIGINT to process group */
...
void sigtstp_handler(int sig)
{
	...
	if (fg != 0)
	{
		if (kill(-fg, SIGTSTP) < 0)	/* send SIGTSTP to process group */
